# ASTEM Attendance Register: User Manual

Version 1.3 Updated Edition

For Administrators, Teachers, Staff, and Parents

## Brand And Status Colours

ASTEM uses a simple colour language throughout the app and this manual.
...
## 1. Welcome To ASTEM Attendance Register

ASTEM Attendance Register is an all-in-one school attendance system designed to make attendance fast, secure, traceable, and easy to report. It supports student attendance, staff attendance, parent visibility, reports, user approvals, QR cards, biometrics, face recognition, school location protection, and administrative setup.
...
| Reports | Opens daily, weekly, monthly, termly, yearly, and individual reports for administrators |
...
Home Page
   ...
   |
   |-- Need reports?
          |-- Reports -> Daily / weekly / monthly / termly / yearly
```
...
3. Add a movement-book reason if the app asks for one because the check-in is very late or the check-out is early.
4. Wait for the success confirmation before moving to the next student.
...
1. Choose a method:
   ...
   - **Staff Face Check-In**.
   - **Staff ID Attendance**.
2. Add a movement-book reason if the app asks for one because the check-in is very late or the check-out is early.
...
- A movement-book reason is required for very late check-ins and early check-outs when the configured policy requires it.
...
When the scanner opens, the top of the screen shows **Scan Student QR**, **Scan Staff QR**, or **Scan Your Staff QR**.

- Tap **Check-in** or **Check-out** to choose the attendance mode.
- Point the camera at the QR card generated by the mobile or web QR card tools.
- The web check-in page uses camera scanning only; the old manual **QR Code Payload** paste box has been removed.
- Signed staff QR codes are validated before staff attendance is recorded.
- If a very late check-in or early check-out requires a reason, choose or type the movement-book reason before saving.
- After success, tap **Scan Again** for the next person where available.
...
| Staff QR Cards | Generates professional printable staff QR cards and PDF/print output |
...
| Attendance Time | Sets late time, close time, timezone, weekend/holiday policy, and triggers absent auto-marking |
...
- Use **Share PNG** or **Export PDF** for a digital or printable copy.
...
Go to **Admin > Staff QR Cards** or **Attendance > Staff QR Generator**.

- Search staff by name, staff ID, email, or role.
- Generate QR cards using the signed professional QR payload used by the mobile app.
- Use **Print / PDF** or **Export PDF** to print staff cards.
- The card preview focuses on the QR image and staff identity. Raw payload text is no longer shown to ordinary check-in users.
...
| Attendance Close Time | After this time, new check-ins are blocked and early check-out policy is calculated | `10:00` |
...
| Yearly Reports | Review a full academic/calendar year |
...
| Attendance % | Attended sessions divided by attended plus absent sessions |
...
Choose Student or Staff
     |
Choose daily / weekly / monthly / termly / yearly
     |
Filter by date, class, term, year, or person
     |
Review summary cards: Present, Late, Attended, Absent, Attendance %
     |
Print / PDF or export PDF if needed
```

### Attendance Totals And Auto-Marked Absence

Recent report pages show total cards above the report table:

| Card | Meaning |
|---|---|
| Present | People checked in before the late time |
| Late | People checked in after the late time |
| Attended | Present plus Late |
| Absent | People auto-marked or recorded as absent for the selected period |
| Attendance % | Attended divided by Attended plus Absent |

Daily reports automatically run the absence marking check for the selected day before showing the summary. Weekly, monthly, termly, yearly, and individual reports use the recorded attendance and absent records in the selected date range. If no absence records exist yet for a person or period, the Absent value remains 0 and the percentage is calculated from the records available.

### Printing Reports

Use **Print / PDF** or **Export PDF** from report pages to create a printable report. The print view contains the selected filters, totals, and report rows without the normal app navigation.
...
| QR code not accepted | QR is wrong, damaged, mismatched, or not from the app card generator | Confirm QR belongs to the right student/staff and class, then regenerate the card if needed |
...
| Attendance | Student/Staff Attendance, QR Scanner, Today's Attendance, Staff ID, Face Check-In, Staff QR Generator, Movement Reasons |
...
| Reports | Daily, Weekly, Monthly, Termly, Yearly, Student Detail, Staff Detail, Staff Yearly |
