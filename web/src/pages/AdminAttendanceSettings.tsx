...
export default function AdminAttendanceSettings() {
  ...
  async function handleSave() {
    ...
    try {
      setSaving(true);
      await saveAttendanceSettings({ lateAfter, closeAfter, timezone });
      ...
      <section className="enterprise-panel overflow-hidden">
        <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-900 px-4 py-3 text-white">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-white/70">Configure late check-in, closing time, and the timezone used across web and mobile workflows.</p>
          </div>
        </div>
      </section>
